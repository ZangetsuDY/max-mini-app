...
function sanitizeDispatcherDivisionId(
  ...
  return /^[a-z0-9][a-z0-9-]{0,63}$/.test(clean)
    ? clean
    : "";
}
...
async function buildAccessResult(
  ...
  const effectiveDispatcherGroups =
    Array.isArray(dispatcherGroups)
      ? dispatcherGroups
      : (await getDispatcherStructure()).groups;

  const activeDispatcherGroupIds =
    new Set(
      effectiveDispatcherGroups.map(
        (group) => group.id
      )
    );
  ...
  */
  ...
  /*
    Контур диспетчера определяется ролями.

    - Разработчик всегда имеет полный доступ.
    - Пользовательская диспетчерская роль может быть ограничена одним
      подразделением или иметь флаг «Все подразделения».
    - Если назначено несколько обычных диспетчерских ролей, доступы
      объединяются.
  */
  ...
  const scopedDispatcherDivisionIds =
    [
      ...new Set(
        effectiveRoles
          .filter(
  ...
  const allDispatcherDivisionIds =
    effectiveDispatcherGroups.map(
      (division) => division.id
    );

  const dispatcherDivisionIds =
    dispatcherAllDivisions
      ? allDispatcherDivisionIds
      : scopedDispatcherDivisionIds;

  const dispatcherDivisionId =
    dispatcherAllDivisions
      ? ""
      : (
          dispatcherDivisionIds[0] ||
          ""
        );

  const dispatcherDivisionNames =
    dispatcherAllDivisions
      ? ["Все подразделения"]
      : dispatcherDivisionIds
          .map(
            (groupId) =>
              dispatcherDivisionNameById(
                groupId,
                effectiveDispatcherGroups
              )
          )
          .filter(Boolean);
...
export async function resolveSessionAccess(
  ...
  // Для серверной проверки сессии всегда читаем актуальное
  // состояние учётной записи. Это позволяет сразу отозвать
  // доступ после удаления пользователя.
  const activeUser =
    await getApplicationUser(
      session.username,
      { force: true }
...
export async function getUsersWithAccess() {
  ...
  const [roles, dispatcherStructure] =
    await Promise.all([
      getRoleDefinitions(),
      getDispatcherStructure()
    ]);
...
export async function createRole({
  ...
  const cleanDispatcherAllDivisions =
  ...
  const cleanDispatcherDivisionName =
    cleanDispatcherAllDivisions
      ? "Все подразделения"
      : dispatcherDivisionNameById(
  ...
  if (
    cleanPanelIds.includes("dispatcher") &&
    !cleanDispatcherAllDivisions &&
    (
      !cleanDispatcherDivisionId ||
      !cleanDispatcherDivisionName
    )
  ) {
    throw new Error(
      "Для диспетчерской роли выберите подразделение или «Все подразделения»"
    );
  }
...
export async function clearUserRoleAssignment(
  username
) {
