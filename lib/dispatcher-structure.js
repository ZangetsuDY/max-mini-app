...
function normalizeState(value) {
  ...
  return {
    ...
    groupOverrides: (Array.isArray(source.groupOverrides)
      ? source.groupOverrides
      : []
    )
      .map((override) => ({
        id: cleanId(override?.id),
        name: cleanName(override?.name),
        description: cleanName(override?.description, 160),
        updatedAt: override?.updatedAt || null,
        updatedBy: override?.updatedBy || null
      }))
      .filter((override) => override.id && override.name),
...
function buildEffectiveStructure(state) {
  ...
  const groupOverrideMap =
    new Map(
      state.groupOverrides.map(
        (override) => [
          override.id,
          override
        ]
...
export async function updateDispatcherGroup({
  groupId,
  name,
  description = "",
  actor
  ...
  if (builtinIds.has(id)) {
    ...
    const existingIndex =
      state.groupOverrides.findIndex(
        (item) => item.id === id
      );
    ...
    if (existingIndex >= 0) {
      ...
      state.groupOverrides.push(
        override
      );
    }
  } else {
    const index =
      state.customGroups.findIndex(
        (group) => group.id === id
      );

    if (index < 0) {
      throw new Error(
        "Пользовательское подразделение не найдено"
      );
    }

    state.customGroups[index] = {
      ...state.customGroups[index],
      name: cleanGroupName,
      description:
        cleanDescription,
